...
object Cycles {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var rev = input
      .map { line =>
        val nodes = line.split(" ")
        (nodes(1), nodes(0))        // (toId, fromId)
      }

    var complete = hops.leftOuterJoin(rev).filter(x => x._2._2.nonEmpty)
      .map(node => (node._2._2.get, node._1))

    logger.info(println(hops.count()))
    logger.info(println(complete.count()))
